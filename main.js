...
global.authFolder = storeSys.fixFileName(`${opts._[0] || ''}AlvinSession`)
    let { state, saveCreds } = await useMultiFileAuthState(path.resolve('./AlvinSession'))
...
async function connectionUpdate(update) {
   ...
   *Tisya Terhubung:*
   ...
   ✓ Description: *Script Punya VnShoop Subcribe vinwert Untuk Info Update Script Ini*.
        `;
        conn.sendMessage("62382341447@s.whatsapp.net", {
            text: infoMsg,
    ...
    if (connection == 'close') {
        console.log(chalk.yellow(`🚩ㅤConnection closed, please delete the folder AlvinSession and rescan the QR code`));
    }
}
...
global.reloadHandler = async function(restatConn) {
  ...
  conn.welcome = '👋𝗛𝗲𝗹𝗹𝗼 @user\n\n𝗪𝗲𝗹𝗰𝗼𝗺𝗲 𝘁𝗼\n◈▻@subject\n\nBaca Deskripsi Group:\n@desc\n*🔖 Pesan*\n_Subcribe Yt Owner Untuk Info Update Script Ini._'
